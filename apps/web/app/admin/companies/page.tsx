...
import { getAdminCompaniesPageData } from "../../../src/admin/companies";
...
export default async function AdminCompaniesPage({ searchParams }: AdminCompaniesPageProps) {
  ...
  const data = await getAdminCompaniesPageData({
    q: params.q ?? null,
  ...
  return (
    <section className="space-y-4">
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        <h2 className="text-xl font-bold text-slate-900">{dictionary.admin.companies.title}</h2>
        <p className="mt-1 text-sm text-slate-600">{dictionary.admin.companies.subtitle}</p>

        <form className="mt-4 grid gap-2 md:grid-cols-[1fr_220px_auto]" action="/admin/companies">
          <input
            name="q"
            defaultValue={params.q ?? ""}
            placeholder={dictionary.admin.companies.filters.search}
          ...
          >
            <option value="">{dictionary.admin.companies.filters.anyStatus}</option>
        ...
        {data.errorCode ? (
          <p className="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs font-medium text-amber-800">
            {dictionary.admin.companies.dataSourceUnavailable}: {data.errorCode}
          </p>
        ) : null}
      </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-slate-50 text-slate-600">
              <tr>
                <th className="px-3 py-2 text-start">{dictionary.admin.companies.columns.companyName}</th>
                <th className="px-3 py-2 text-start">{dictionary.admin.companies.columns.representative}</th>
                ...
                <th className="px-3 py-2 text-start">{dictionary.admin.companies.columns.verification}</th>
                <th className="px-3 py-2 text-start">{dictionary.admin.users.columns.listings}</th>
                <th className="px-3 py-2 text-start">{dictionary.admin.companies.columns.joinedAt}</th>
              ...
              {data.rows.map((row) => (
                <tr key={row.userId} className="border-t border-slate-100">
                  ...
                  <td className="px-3 py-2 text-slate-700">
                    {dictionary.admin.verifications.status[row.verificationStatus as keyof typeof dictionary.admin.verifications.status] ??
                      row.verificationStatus}
                  </td>
                  <td className="px-3 py-2 text-slate-700">{row.listingsCount}</td>
                  <td className="px-3 py-2 text-slate-700">{row.joinedAt ? formatDateTime(row.joinedAt, language) : "—"}</td>
                  <td className="px-3 py-2">
                    <Link href={`/admin/companies/${row.userId}`} className="text-xs font-semibold text-brand hover:underline">
                      {dictionary.admin.companies.actions.view}
                    </Link>
                  </td>
                </tr>
              ))}
        ...
        <div className="flex items-center gap-2">
          <Link
            href={(() => {
              ...
              return `/admin/companies?${next.toString()}`;
            })()}
